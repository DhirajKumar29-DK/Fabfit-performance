...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let service = await getServiceBySlug(serviceSlug);

  if (!service) {
    const dummyService = homeData.services.items.find((item: any) => item.id.toString() === serviceSlug || item.slug === serviceSlug);
    if (dummyService) {
      service = {
  ...
  const metrics = [
    { icon: Clock, title: "DURATION", value: service.duration },
    { icon: Calendar, title: "SESSIONS", value: service.sessions },
    { icon: BarChart, title: "LEVEL", value: service.level },
    { icon: Dumbbell, title: "EQUIPMENT", value: service.equipment }
  ];

  return (
    <div className="bg-[#020202] min-h-screen text-white pt-24 pb-0 selection:bg-[#d4af37] selection:text-black">
      ...
      <div className="relative border-b border-white/5 pb-20">
        <div className="container mx-auto px-4 md:px-8 relative z-10 flex flex-col lg:flex-row items-center pt-8">
          ...
          <div className="w-full lg:w-1/2 lg:pr-16 z-20">
            ...
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-zinc-200 to-zinc-500 leading-[1.1] uppercase mb-6 tracking-tighter drop-shadow-sm">
              {service.title}
            </h1>
            <p className="text-lg md:text-xl font-medium text-zinc-300 mb-6 max-w-lg leading-snug">
              {service.shortDescription}
            </p>
            <p className="text-zinc-500 text-sm mb-10 max-w-md leading-relaxed font-medium whitespace-pre-line">
              {service.description}
            </p>
      ...
      {features.length > 0 && (
        <section className="py-20 container mx-auto px-4 md:px-8 relative z-10">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <h2 className="font-heading text-3xl md:text-5xl font-black text-white uppercase tracking-tighter mb-6">
              WHAT'S INCLUDED
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((item: any, index: number) => {
              ...
              return (
                <div key={index} className="group relative bg-[#080808] border border-white/5 rounded-2xl p-8 flex flex-col items-center text-center transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.5)] overflow-hidden">
                  ...
                  <h3 className="relative z-10 text-lg font-black text-white mb-3 tracking-wide">{item.title}</h3>
        ...
        <section className="py-20 container mx-auto px-4 md:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row gap-12 items-center max-w-6xl mx-auto">
            ...
            <div className="w-full lg:w-1/2">
              ...
              <h2 className="font-heading text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-6 leading-tight">
                ABOUT THIS PROGRAM
              </h2>
        ...
        <section className="container mx-auto px-4 md:px-8 pb-24 mt-12 relative z-10">
          <div className="relative rounded-3xl overflow-hidden bg-[#050505] border border-white/5 flex flex-col lg:flex-row lg:h-[340px] shadow-[0_0_40px_rgba(0,0,0,0.8)] group hover:border-[#d4af37]/30 hover:shadow-[0_0_40px_rgba(212,175,55,0.15)] transition-all duration-700 max-w-5xl mx-auto">
            ...
            <div className="w-full lg:w-1/2 p-6 md:p-10 lg:p-12 flex flex-col justify-center relative z-10">
              ...
              {service.ctaTitle && (
                <h2 className="font-heading text-2xl md:text-3xl font-black text-white mb-4 uppercase tracking-tight leading-tight drop-shadow-sm line-clamp-2">
                  {service.ctaTitle}
                </h2>
              )}
